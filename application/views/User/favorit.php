...
<div class="container container-kelola">
  <h2 class="fw-bold text-center mb-4">FAVORIT RESEP</h2>
  ...
  <?php if (empty($favorites)): ?>
    <p class="text-center">Anda belum menyimpan resep favorit.</p>
    <?php else: ?>
  <div class="row">
    <?php foreach ($favorites as $r): ?>
    <div class="card card-kelola mb-3 col-md-10 mx-auto">
      <div class="row g-0 align-items-center">
        <div class="col-md-4">
          <img src="<?= base_url('assets/recipe/foto/' . $r['foto']); ?>" class="img-fluid" alt="<?= $r['judul']; ?>"
          style="height: 200px; border-radius: 25px; object-fit: cover;">
        </div>
        <div class="col-md-8">
          <div class="card-body">
            <h5 class="card-title"><?= $r['judul']; ?></h5>
            <div>
              <a href="<?= base_url('dashboard/halaman_resep/' . $r['id_resep']); ?>" class="btn btn-warning btn-sm" style="border-radius: 25px; margin-right: 10px;">Lihat Resep</a>
              <a href="<?= base_url('dashboard/hapus_favorit/' . $r['id_resep']); ?>" class="btn btn-secondary btn-sm" 
               onclick="return confirm('Apakah Anda yakin ingin menghapus resep favorit ini?')">Hapus dari Favorite</a>
            </div>
          </div>
        </div>
      </div>
